...
def draw():
    ...
    # Inicia a definição do primeiro triângulo
    glBegin(GL_TRIANGLES)
    glColor3f(1, 0, 0)  # Define a cor do primeiro triângulo (Vermelho)
    ...
    glEnd()  # Finaliza a definição do primeiro triângulo

    # === EX11) Adicionando um segundo triângulo deslocado no eixo X ===
    # Inicia a definição do segundo triângulo
    glBegin(GL_TRIANGLES)
    glColor3f(0, 1, 0)  # Define a cor do segundo triângulo (Verde)
    glVertex3f(0 + 2, 1, 0)  # Vértice superior deslocado em +2 no X
    glVertex3f(-1 + 2, -1, 0)  # Vértice inferior esquerdo deslocado em +2 no X
    glVertex3f(1 + 2, -1, 0)  # Vértice inferior direito deslocado em +2 no X
    glEnd()  # Finaliza a definição do segundo triângulo
